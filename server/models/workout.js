...
module.exports = {
  addWorkout: (req, res) => {
    ...
    if (decoded != null) {
      db.query(
        `INSERT INTO ${prefix}Workouts (created_at, Owner_id, Start_Time, End_Time, Exercise_Type, Note, Distance) 
  ...
  getWorkouts: (req, res) => {
    ...
    if (decoded != null) {
      db.query(
        `SELECT * FROM ${prefix}Workouts WHERE Owner_id=${decoded[0].id} ORDER BY Start_Time DESC`,
        (err, results, fields) => {
          if (err) throw err;
          res.send(results);
          //res.send("Successfully added exercise!")
  ...
  searchWorkouts: (req, res) => {
    let decoded = auth.verifyToken(req);
    //console.log(decoded);
    // console.log("request" + req.body.message);
    if (decoded != null) {
      db.query(
        `SELECT * FROM ${prefix}Workouts WHERE Note LIKE '%${req.body.message}%'`,
        (err, results, fields) => {
          if (err) throw err;
          res.send(results);
          //res.send("Successfully added exercise!")
